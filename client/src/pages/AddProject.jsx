...
const AddEmployee = () => {
    ...
    const employeeOptions = [
        { value: 0, text: "--Қызметкерді таңдаңыз --", role: 'employee', id: '' }
    ];

    const clientOptions = [
        { value: 0, text: "--Клиентті таңдаңыз --", role: 'client', id: '' }
    ];

    const managerOptions = [
        { value: 0, text: "--Менеджерді таңдаңыз --", role: 'manager', id: '' }
    ];
            ...
            <Row>
                ...
                <Row>
                    ...
                    <Col md={12} style={{ margin: '12px auto' }}>
                        ...
                        <select
                            selected={manager}
                            onChange={event => { setManager(managerOptions[event.target.value].id) }}
                            className="form-control-input select-input flex-fill d-block w-100"
                            style={{
                                backgroundColor: 'white'
                            }}>
                            {managerOptions.map((option) => option?.role == 'manager' && (
                    ...
                    <Col md={12} style={{ margin: '12px auto' }}>
                        ...
                        <select
                            selected={employee}
                            onChange={event => { setEmployee(employeeOptions[event.target.value]?.id) }}
                            className="form-control-input select-input flex-fill d-block w-100"
                            style={{
                                backgroundColor: 'white'
                            }}>
                            {employeeOptions?.map((option, i) => option?.role == 'employee' && (
                                <option key={option.value} value={option.value}>
                                    {option.text}
                                </option>
                            ))}
                    ...
                    <Col md={12} style={{ margin: '12px auto' }}>
                        ...
                        <select
                            selected={client}
                            onChange={event => { setClient(clientOptions[event.target.value].id) }}
                            className="form-control-input select-input flex-fill d-block w-100"
                            style={{
                                backgroundColor: 'white'
                            }}>
                            {clientOptions?.map((option, i) => option?.role == 'client' && (
                                <option key={option.value} value={option.value}>
                                    {option.text}
                                </option>
                            ))}
